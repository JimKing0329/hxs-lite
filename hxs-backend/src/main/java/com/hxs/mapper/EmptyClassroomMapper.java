...
public interface EmptyClassroomMapper extends BaseMapper<EmptyClassroom> {
    ...
    @Insert("<script>" +
            "INSERT INTO empty_classroom(class_id, class_name, campus_name, class_category, building, week_number, weekday) VALUES " +
            "<foreach collection='list' item='item' separator=','>" +
            "(#{item.classId}, #{item.className}, #{item.campusName}, #{item.classCategory}, #{item.building}, #{item.weekNumber}, #{item.weekday})" +
            "</foreach>" +
            "</script>")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    void insertClassroom(@Param("list") List<EmptyClassRoomItem> list);
    ...
    @Insert("<script>" +
            ...
            "</foreach>" +
            "</script>")
    void insertEmptySession(@Param("classroomId") Long classroomId,
                            @Param("sessions") List<Integer> sessions);
    ...
    @Select("SELECT id FROM empty_classroom WHERE week_number = #{week} AND weekday = #{weekday} AND class_id = #{classId}")
    Long selectExistingClassroomId(@Param("week") Integer week,
